...
- The ExperienceWelcome homepage static replication phase uses English as the visual fidelity baseline. The shared Navbar, Header, HowItWorks, CustomerStories, Footer, and homepage metadata are localized through message namespaces; remaining section copy is migrated incrementally.
...
- `src/i18n/messages/*.json` includes `Navbar`, `Header`, `HowItWorks`, `CustomerStories`, `Footer`, and `HomeMetadata` namespaces. `src/app/[locale]/(home)/page.tsx` reads those messages server-side and passes plain copy objects into reusable homepage components.
...
- **2026-07-03-how-it-works-localization**: Add HowItWorks to the homepage copy contract.
  Status: Accepted.
  Context: The How It Works section is now part of the completed homepage route and has meaningful title, intro, CTA, and three step-copy blocks.
  Decision: Add a `HowItWorks` namespace to every supported locale and shape it through `src/i18n/homepage-copy.ts` before passing it to `HomepageHowItWorks`.
  Consequences: The section remains server-rendered and localized like Header, CustomerStories, and Footer, while the exported media stays decorative.
